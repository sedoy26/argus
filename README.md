...
5. **Watch paths (important)** — GitHub autodeploy “**skipped / no changes to watched files**” uses **`watchPatterns` stored on the Railway service instance** (often from the UI at import), which can **ignore** what you put in `railway.toml` alone. This repo’s `scripts/railway/railway.deploy.config.json` lists **`watchPatterns` per service** (gateway uses **`["**"]`** so it never skips); **`bun run scripts/railway/configure-github-autodeploy.ts --skip-connect`** sends them via **`serviceInstanceUpdate`** together with **`railwayConfigFile`**. Re-run that after forking or if skips return. In the Railway UI you can also clear or edit **Watch paths** on each service to match.
