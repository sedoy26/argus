...
5. **Watch paths** — `dashboard` and `signal-api` use narrow **`watchPatterns`** so unrelated pushes skip them. **`argus-gateway`** (`platform/ens-resolver/railway.toml`) intentionally uses **`["**"]`** so Railway never reports *No changes to watched files* for that service (we hit false skips with `/platform/ens-resolver/**`). Gateway builds are cheap; you can narrow again once Railway matching is reliable.
